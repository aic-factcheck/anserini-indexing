...
def evaluate_retrieval(args):
    evidences = {}
    top_ks = [1, 5, 10, 25, 50, 100, 500]
    fully_supported = {k: 0 for k in top_ks}
    oracle_accuracy = {k: 0 for k in top_ks}
    num_queries = 0
    num_supported_queries = 0
    k1, b = '', ''
    k1_s = args.run_file.find('k1_')
    b_s = args.run_file.find('b_')

    k1 = args.run_file[k1_s+3:k1_s+6]
    b = args.run_file[b_s+2:b_s+5]
    ...
    out = f"k1 = {k1}\tb = {b}\n"
    out+='k\tFully Supported\tOracle Accuracy\n'
    for k in top_ks:
        out += f"{k}\t{fully_supported[k] / num_supported_queries:.4f}\t{oracle_accuracy[k] / num_queries:.4f}\n"
    print(out)
